...
document.body.insertBefore(modelSelector, chatInput);
...
async function sendMessage() {
  ...
  try {
    ...
    let aiResponse = "";
    let aiMessageElement = document.createElement("p"); // Create a new paragraph for AI response
    aiMessageElement.innerHTML = `<strong>AI:</strong> `;
    chatOutput.appendChild(aiMessageElement); // Append to chat before streaming starts
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (parsed.word) {
            ...
            aiMessageElement.innerHTML = `<strong>AI:</strong> ${aiResponse}`; // Update existing AI message
          }
        } catch (error) {
